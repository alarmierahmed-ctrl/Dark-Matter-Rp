-- Dark Matter - Supabase database setup
...
create policy "supervisor self or owner read"
...
create policy "owner adds supervisor"
on public.supervisors for insert to authenticated
with check (public.is_site_owner());

drop policy if exists "owner removes supervisor" on public.supervisors;
create policy "owner removes supervisor"
on public.supervisors for delete to authenticated
...
create policy "buyer or staff reads orders"
